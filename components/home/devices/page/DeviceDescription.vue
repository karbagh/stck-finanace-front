    ...
    <b-list-group flush>
      ...
      <b-list-group-item>
        <section class="discount" v-if="device.discount">
          ...
          <h4 class="h4 text-decoration-line-through text-danger">{{ device.oldPrice }}
            <AMD/>
          </h4>
        </section>
        <h4 class="h2">{{ device.price }}
          <AMD/>
        </h4>
      </b-list-group-item>
      <b-list-group-item>
        ...
        <h5 v-bind:class="[device.inStock ? 'text-success' : 'text-danger']">
          {{ device.inStock ? $t('in_stock.true') : $t('in_stock.false') }}</h5>
      </b-list-group-item>
